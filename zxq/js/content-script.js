...
document.addEventListener('DOMContentLoaded', function()
{
	...
	else if(location.href == 'https://www.baidu.com/'){
		function removeBaiduAD(){
			// if(document.getElementById('my_css')) return;
			var mytemp=document.createElement('style');
			mytemp.id='my_css';
			(document.head||docuemnt.body).appendChild(mytemp);
			var mycss = `
				#s_wrap{opacity:0}
				...
				#u_sp{display:none}
				/* 移除百度右侧广告 */
				#content_right{opacity:0}
				/* 覆盖整个屏幕的相关推荐 */
				.rrecom-btn-parent{opacity:0}'
				/* 难看的按钮 */
				.result-op.xpath-log{opacity:0 !important;}
			`
			mytemp.innerHTML = mycss;
			console.log("首页百度已注入自定义的css")
			// alert("百度首页注入")
			mytemp.addEventListener('DOMNodeRemoved', function(e)
			{
				console.log('首页百度自定义CSS被移除，重新注入！');
				// alert("重新注入");
				// alert("百度首页重新注入")
				removeBaiduAD();
			});
